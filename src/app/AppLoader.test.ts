...
import { getActiveAppConfig, getActiveAppName, AVAILABLE_APPS } from './AppLoader';

// ─── Scenario 1: getActiveAppConfig returns the mocked AppConfig ──────────────

describe(
    'Scenario 1: getActiveAppConfig returns the build-time AppConfig from virtual module',
    () => {
        it(
...
describe(
    'Scenario 2: AVAILABLE_APPS is a frozen read-only registry',
    () => {
        it(
...
describe(
    'Scenario 3: getActiveAppName returns the ACTIVE_APP_NAME from the virtual module',
    () => {
        it(
