...
function profileDerivationSnippet(sourceApp = "__appConfig") {
  return [
...
export function generateBridgeFiles(config, cwd = process.cwd()) {
  ...
  const content = [
    ...
    ``,
    `export const AVAILABLE_APPS = Object.freeze(${JSON.stringify(config.availableApps)});`,
    ``,
