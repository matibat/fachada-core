...
describe(
    'Scenario 1: buildMergedStaticPaths includes config-declared subsections in the result',
    () => {
        beforeEach(() => {
...
describe(
    'Scenario 2: buildMergedStaticPaths passes the correct existingPaths Set to collectMarkdownPages',
    () => {
        beforeEach(() => {
            vi.mocked(collectMarkdownPages).mockClear();
            vi.mocked(collectMarkdownPages).mockReturnValue({
